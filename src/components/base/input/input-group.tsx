...
const InputPrefix = ({ isDisabled, children, ...props }: InputPrefixProps) => (
  <span
    {...props}
    className={cx(
      'text-md text-tertiary ring-border-primary flex cursor-default rounded-lg shadow-xs ring-1 ring-inset',
      // Styles when the prefix is within an `InputGroup`
      'in-data-input-wrapper:in-data-leading:-mr-px in-data-input-wrapper:in-data-leading:rounded-l-lg in-data-input-wrapper:in-data-leading:rounded-r-none',
      'in-data-input-wrapper:in-data-trailing:-ml-px in-data-input-wrapper:in-data-trailing:rounded-l-none in-data-input-wrapper:in-data-trailing:rounded-r-lg',
...
export const InputGroup = ({
  ...
  return (
    <TextField
      ...
      inputClassName={cx(
        paddings[size].input,
        // Apply rounded styles based on the presence of leading or trailing elements
        // Use arbitrary values with !important to override default rounded-lg
        hasLeading &&
          '[border-top-left-radius:0!important] [border-bottom-left-radius:0!important] [border-top-right-radius:0.5rem!important] [border-bottom-right-radius:0.5rem!important]',
        hasTrailing &&
      ...
      {state => (
        <>
          ...
          <div
            ...
            className={cx(
              'group bg-primary relative flex h-max w-full flex-row justify-center rounded-lg transition-all duration-100 ease-linear',

              // Only apply focus ring when child is select and input is focused
              'has-[&>select]:ring-border-primary has-[&>select]:has-[input:focus]:ring-border-brand has-[&>select]:shadow-xs has-[&>select]:ring-1 has-[&>select]:ring-inset has-[&>select]:has-[input:focus]:ring-2',

              state.isDisabled &&
                'has-[&>select]:bg-disabled_subtle has-[&>select]:ring-border-disabled cursor-not-allowed',
              state.isInvalid &&
